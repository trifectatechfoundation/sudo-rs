...
pub(crate) fn sudo_call<T>(
    ...
    eprintln_ignore_io_error!("{:?}", cur_groups);
    eprintln_ignore_io_error!("{:?}", target_groups);
    #[cfg(test)]
    if (target_user.uid, target_group.gid) == (cur_user_id, cur_group_id) &&
        target_groups.iter().filter(|x| x != target_group.gid) == 
        cur_groups.iter().filter(|x| x != cur_group_id) 
    {
