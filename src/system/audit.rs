...
use crate::sudoers::{Identifier, PeerSpec, UserSpecifier};
...
#[cfg(feature = "unstable-remote-sudoers")]
fn check_user(peer_uid: libc::uid_t, user_spec: &UserSpecifier) -> io::Result<()> {
    match user_spec {
        UserSpecifier::User(Identifier::Name(name)) => {
        ...
        UserSpecifier::User(Identifier::ID(uid)) => {
            if peer_uid != *uid {
                return Err(io::Error::new(
                    io::ErrorKind::PermissionDenied,
                    format!(
        ...
        _ => {
            return Err(io::Error::new(
                ...
                "expected user specifier, not group",
            ));
        }
    }
    Ok(())
...
#[cfg(feature = "unstable-remote-sudoers")]
fn check_group(peer_gid: libc::gid_t, group_spec: Option<&UserSpecifier>) -> io::Result<()> {
    let Some(group_spec) = group_spec else {
        return Ok(());
    };
    ...
    match group_spec {
        ...
        UserSpecifier::Group(Identifier::ID(gid)) => {
            if peer_gid != *gid {
                return Err(io::Error::new(
                    io::ErrorKind::PermissionDenied,
                    format!(
        ...
        UserSpecifier::NonunixGroup(_) => {
            return Err(io::Error::new(
                ...
                "non-POSIX groups are not supported for socket peer validation",
            ));
        }
