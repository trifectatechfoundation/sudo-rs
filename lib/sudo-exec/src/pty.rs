...
impl<'a> ExecClosure<'a> {
    fn new(
        ...
        events.add_read_event(&ec.sigint_recv.as_raw_fd(), |ec, events| {
            ec.handle_recv_signal(SIGINT, events)
        });
        events.add_read_event(&ec.sigquit_recv.as_raw_fd(), |ec, events| {
            ec.handle_recv_signal(SIGQUIT, events)
        });
        events.add_read_event(&ec.sigtstp_recv.as_raw_fd(), |ec, events| {
            ec.handle_recv_signal(SIGTSTP, events)
        });
        events.add_read_event(&ec.sigterm_recv.as_raw_fd(), |ec, events| {
            ec.handle_recv_signal(SIGTERM, events)
        });
        events.add_read_event(&ec.sighup_recv.as_raw_fd(), |ec, events| {
            ec.handle_recv_signal(SIGHUP, events)
        });
        events.add_read_event(&ec.sigalrm_recv.as_raw_fd(), |ec, events| {
            ec.handle_recv_signal(SIGALRM, events)
        });
        events.add_read_event(&ec.sigusr1_recv.as_raw_fd(), |ec, events| {
            ec.handle_recv_signal(SIGUSR1, events)
        });
        events.add_read_event(&ec.sigusr2_recv.as_raw_fd(), |ec, events| {
            ec.handle_recv_signal(SIGUSR2, events)
        });
        events.add_read_event(&ec.sigchld_recv.as_raw_fd(), |ec, events| {
    ...
    fn suspend(&mut self, signal: c_int) -> c_int {
        ...
        match signal {
            ...
            SIGSTOP | SIGTSTP | _ => {
                ...
                if signal != SIGSTOP {
                    // FIXME: ogsudo overrides the handler for `signal` here.
                }
                ...
                if signal != SIGSTOP {
                    // FIXME: ogsudo restores the handler for `signal` here.
                }
